...
namespace Unity.MLAgentsExamples
{
    ...
    public class ModelOverrider : MonoBehaviour
    {
        ...
        const string k_CommandLineExitDelaySeconds = "--mlagents-exit-delay-seconds";
        ...
        float m_ExitDelaySeconds = GameConfig.InferenceExitDelaySeconds;
        bool m_ExitSequenceStarted;
        ...
        void GetAssetPathFromCommandLine()
        {
            ...
            for (var i = 0; i < args.Length; i++)
            {
                ...
                else if (args[i] == k_CommandLineExitDelaySeconds && i < args.Length - 1)
                {
                    float parsedDelay;
                    if (float.TryParse(args[i + 1], out parsedDelay))
                        m_ExitDelaySeconds = parsedDelay;
                }
            }
        ...
        void TriggerExit()
        {
            if (m_ExitSequenceStarted) return;
            m_ExitSequenceStarted = true;
            ...
            {
                foreach (var ctrl in FindObjectsByType<SoccerEnvController>())
                    ctrl.StopBallGeneration();
                StartCoroutine(DelayedQuit(m_ExitDelaySeconds));
            }
        }

        System.Collections.IEnumerator DelayedQuit(float delay)
        {
            Debug.Log($"[ModelOverrider] Exiting in {delay}s...");
